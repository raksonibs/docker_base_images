...
if [ $? -gt 0 ] ; then
  ...
  echo "⚠️   Unable to retrieve the current VERSION for the base image from the github repo.  ⚠️"
  ...
  if [ "$(version "$LATEST_VERSION")" -gt "$(version "$IMAGE_VERSION")" ] ; then
    ...
    if [ $? -gt 0 ] ; then
      # error getting version...
      echo "Unable to retrieve the CHANGELOG.md for the base image from the github repo."
    else
      echo "################################################################################"
